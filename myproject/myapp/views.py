...
client = MongoClient("mongodb+srv://neocitymakerlab:87RtyP8430cIeWkR@cluster.crddfjt.mongodb.net/?retryWrites=true&w=majority", tlsCAFile=certifi.where(), tls=True, server_api=ServerApi('1'))
db = client["inventory"]
all_items = db["items"]
all_tags = db["tags"]

student_db = client["tracking"]
student_all_items = student_db["ids"]
...
def run_student_view(request):
    ...
    all_documents_list = list(student_all_items.find())\

    return render(request, 'students.html', {'documents': all_documents_list})
...
def add_student(request):
    requests = [request.POST.get('id'), request.POST.get('name'), request.POST.get('in_time'), request.POST.get('cert_level'), request.POST.get('reason')]
    
    id, name, in_time, cert_level, reason = requests
    document = {"id": id, "name": name, "in_time": in_time, "cert_level": cert_level, "reason": reason}

    student_all_items.insert_one(document)
